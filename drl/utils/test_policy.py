...
def run_policy(env, policy, max_ep_len=None, num_episodes=100, render=True):
    ...
    while n < num_episodes:
        if render:
            env.render()
            time.sleep(1e-3)
        action = policy(core.to_tensor(state).unsqueeze(0))
        state, reward, done, _ = env.step(core.to_numpy(action).flatten())
        ep_return += reward
        ep_len += 1
...
def run_gaussian_policy(env, policy, max_ep_len=None, num_episodes=100, render=True):
    ...
    while n < num_episodes:
        if render:
            env.render()
            time.sleep(1e-3)
        _, _, _, action, _ = policy.sample(core.to_tensor(state).unsqueeze(0))
...
if __name__ == '__main__':
    ...
    if 'sac' in args.fpath:
        run_gaussian_policy(env, policy, args.len, args.episodes, not(args.norender))
    else:
